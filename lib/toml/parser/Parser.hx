	...
	private function evaluate( ... tokens : toml.token.Metadata) : Result<Dynamic,Error> {
		...
		switch(tokens[0].token) {
			...
			case Word(text):
				...
				// checks for a comment, allows for a space between the "word" and the hash
				if (tokens.length != 1 && (tokens[1].token == Hash || tokens[2].token == Hash)) {
					var comment = [ ];
					while(tokens.length > 1) comment.unshift(tokens.pop());
				}
