				...
				kind: FFun({
					...
					expr: macro {
						switch(toml.Toml.parse(text, source)) {
							...
							case Ok(loaded):
								var flat = toml.Tools.flatten(loaded);
								for (k => v in flat) {
									...
									if (olddata == null) throw('$k does not exist, cannot write to fields that dont exist: $source');
										...
										var ok = false;
										switch ([Type.typeof(olddata), Type.typeof(v)]) {
											case [TClass(a), TClass(b)]: if(a == b) ok = true;
											case [a, b]: if(a == b) ok = true;
										}
										if(!ok) throw ('$k from $source is a different type than origin: ${Type.typeof(olddata)} vs ${Type.typeof(v)}');
									}
